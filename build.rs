...
/// The Luau declarations checked into the repository for hermetic builds.
const BUILTIN_SOURCES: &[&str] = &[
    "assets/builtin_definitions.luau",
    "assets/builtin_definitions_extra.luau",
];

/// The Rust module emitted into Cargo's build output directory.
const GENERATED_FILE: &str = "builtin_definitions.rs";

/// Parses the checked-in declarations and writes their owned Rust representation.
fn main() {
    generate_builtin_definitions().expect("failed to generate builtin definitions");
}

/// Runs `luau-ast`, lowers its AST, and writes the generated Rust expressions.
fn generate_builtin_definitions() -> Result<(), String> {
    println!("cargo:rerun-if-env-changed=LUAU_AST");
    ...
    for source in BUILTIN_SOURCES {
        ...
        statements.extend(parse_luau_ast(&source_path)?.root.body);
    }
    ...
    fs::write(output_dir.join(GENERATED_FILE), generated)
        .map_err(|error| format!("could not write {GENERATED_FILE}: {error}"))?;

    Ok(())
}

/// Invokes the official parser and deserializes the JSON AST through derived models.
fn parse_luau_ast(source_path: &Path) -> Result<AstDocument, String> {
    let executable = env::var_os("LUAU_AST").unwrap_or_else(|| "luau-ast".into());
    let output = Command::new(&executable)
        .arg(source_path)
        .output()
        .map_err(|error| {
            format!(
                "could not run {:?}; install luau-ast or set LUAU_AST: {error}",
                executable
            )
        })?;
...
/// A generated builtin global and its independently instantiated namespace fields.
#[derive(Debug)]
struct GeneratedBuiltin {
    ...
    /// Schemes for direct namespace field access.
    fields: Vec<GeneratedField>,
}

/// A generated namespace field scheme.
#[derive(Debug)]
struct GeneratedField {
    /// Field name.
    name: String,
    /// Independently bound field type.
    scheme: GeneratedScheme,
}
...
impl BuiltinLowerer {
    ...
    /// Lowers a global annotation and independently binds direct table fields.
    fn lower_global_declaration(
        &self,
        global: &AstDeclareGlobal,
    ) -> Result<GeneratedBuiltin, String> {
        let fields = match &global.luau_type {
            AstType::Table { props, .. } => props
                .iter()
                .map(|property| {
                    Ok(GeneratedField {
                        name: property.name.clone(),
                        scheme: self.lower_scheme(&property.prop_type)?,
                    })
                })
...
/// Renders all lowered definitions as one included Rust function.
fn render_definitions(definitions: &[GeneratedBuiltin]) -> String {
    let mut output = String::from(
        "/// Returns the builtin definitions generated from Luau's parser AST.\n\
         fn generated_builtin_definitions() -> Vec<BuiltinDefinition> {\n    vec![\n",
    );
    for definition in definitions {
        output.push_str("        BuiltinDefinition { name: ");
        output.push_str(&rust_string(&definition.name));
        output.push_str(".to_owned(), scheme: ");
        output.push_str(&render_scheme(&definition.scheme));
        output.push_str(", fields: vec![");
        for field in &definition.fields {
            output.push_str("BuiltinFieldDefinition { name: ");
            output.push_str(&rust_string(&field.name));
            output.push_str(".to_owned(), scheme: ");
            output.push_str(&render_scheme(&field.scheme));
            output.push_str(" },");
        }
        output.push_str("] },\n");
    }
...
/// Renders one pre-bound owned type scheme.
fn render_scheme(scheme: &GeneratedScheme) -> String {
    let binders = scheme
        .binders
        .iter()
        .map(|binder| match binder {
            GeneratedBinder::Type(name) => format!(
                "BuiltinBinderDefinition::Type({}.to_owned())",
                rust_string(name)
            ),
            GeneratedBinder::Pack(name) => format!(
                "BuiltinBinderDefinition::Pack({}.to_owned())",
                rust_string(name)
            ),
        })
        .collect::<Vec<_>>()
        .join(", ");
    format!(
        "BuiltinSchemeDefinition {{ binders: vec![{binders}], body: {} }}",
        render_type(&scheme.body)
    )
}

/// Renders one recursive owned type expression.
fn render_type(ty: &GeneratedType) -> String {
    match ty {
        ...
        GeneratedType::Named(name) => {
            format!("BuiltinType::Named({}.to_owned())", rust_string(name))
        }
        GeneratedType::Generic(name) => {
            format!("BuiltinType::Generic({}.to_owned())", rust_string(name))
        }
        GeneratedType::Literal(GeneratedLiteral::String(value)) => format!(
            "BuiltinType::Literal(TypeLiteral::String({}.to_owned()))",
            rust_string(value)
        ),
        GeneratedType::Literal(GeneratedLiteral::Boolean(value)) => {
            format!("BuiltinType::Literal(TypeLiteral::Boolean({value}))")
        }
        GeneratedType::Table { fields, indexer } => {
            let fields = fields
                .iter()
                .map(|(name, ty)| {
                    format!("({}.to_owned(), {})", rust_string(name), render_type(ty))
                })
                .collect::<Vec<_>>()
                .join(", ");
            ...
            format!("BuiltinType::Table {{ fields: vec![{fields}], indexer: {indexer} }}")
        }
        ...
        GeneratedType::Union(types) => format!(
            "BuiltinType::Union(vec![{}])",
            types.iter().map(render_type).collect::<Vec<_>>().join(", ")
        ),
        GeneratedType::Intersection(types) => format!(
            "BuiltinType::Intersection(vec![{}])",
            types.iter().map(render_type).collect::<Vec<_>>().join(", ")
        ),
    }
}

/// Renders one fixed-prefix owned type pack.
fn render_pack(pack: &GeneratedPack) -> String {
    ...
    let tail = match &pack.tail {
        Some(GeneratedPackTail::Homogeneous(ty)) => format!(
            "Some(BuiltinPackTail::Homogeneous(Box::new({})))",
            render_type(ty)
        ),
        Some(GeneratedPackTail::Generic(name)) => format!(
            "Some(BuiltinPackTail::Generic({}.to_owned()))",
            rust_string(name)
        ),
        None => "None".to_owned(),
    };
    format!("BuiltinPack {{ head: vec![{head}], tail: {tail} }}")
}
