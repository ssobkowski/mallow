...
fn concat_expr_range(start: u8, end: u8) -> HilExpr {
    ...
    let mut expr = HilExpr::Local(end);
...
impl<'a> Lifter<'a> {
    ...
    fn run(mut self) -> Vec<Spanned<HilStmt>> {
        while let Some(instr) = self.next() {
            match &instr {
                Instr::Nop => {}
                Instr::LoadNil { reg } => self.assign(HilExpr::Local(*reg), HilExpr::Nil),
                Instr::LoadB { reg, value, .. } => {
                    self.assign(HilExpr::Local(*reg), HilExpr::Bool(*value))
                }
                Instr::LoadN { reg, value } => {
                    self.assign(HilExpr::Local(*reg), HilExpr::Number(*value as f64))
                }
                Instr::LoadK { reg, index } => {
                    self.assign(HilExpr::Local(*reg), self.const_expr(*index as usize));
                }
                Instr::Move { dest, src } => {
                    self.assign(HilExpr::Local(*dest), HilExpr::Local(*src))
                }
                Instr::GetGlobal { dest, key, .. } => {
                    self.assign(
                        HilExpr::Local(*dest),
                        HilExpr::Global(self.const_string(*key as usize)),
                    );
                }
                ...
                Instr::GetUpval { dest, upval } => {
                    self.assign(HilExpr::Local(*dest), HilExpr::Upval(*upval))
                }
                Instr::SetUpval { src, upval } => {
                    self.assign(HilExpr::Upval(*upval), HilExpr::Local(*src))
                }
                Instr::GetImport { dest, index, path } => {
                    let name = self
                        .decode_import_path(*path)
                        .unwrap_or_else(|| format!("import_k{}", index));
                    self.assign(HilExpr::Local(*dest), HilExpr::Import(name));
                }
                ...
                Instr::GetTableKS {
                    ...
                    self.assign(
                        HilExpr::Local(*dest),
                        HilExpr::GetField {
                            obj: Box::new(HilExpr::Local(*table)),
                            field: key_str,
                        },
                    );
                }
                Instr::GetTable { dest, table, key } => {
                    self.assign(
                        ...
                        HilExpr::GetIndex {
                            obj: Box::new(HilExpr::Local(*table)),
                            index: Box::new(HilExpr::Local(*key)),
                        },
                    );
                }
                ...
                Instr::SetTableKS {
                    ...
                    self.push(HilStmt::SetField {
                        table: *table,
                        key: self.const_string(*key as usize),
                        value: HilExpr::Local(*src),
                    });
                }
                ...
                | Instr::Or { dest, a, b }) => {
                    self.assign(
                        HilExpr::Local(*dest),
                        HilExpr::Binary {
                            lhs: Box::new(HilExpr::Local(*a)),
                            op: binop_for_instr(instr),
                            rhs: Box::new(HilExpr::Local(*b)),
                        },
                    );
                }
                ...
                Instr::Concat { dest, a, b } => {
                    self.assign(HilExpr::Local(*dest), concat_expr_range(*a, *b))
                }
                ...
                Instr::Not { dest, reg } => self.assign(
                    ...
                    HilExpr::Unary {
                        op: UnOp::Not,
                        expr: Box::new(HilExpr::Local(*reg)),
                    },
                ),
    ...
    fn lift_call(&mut self, func: u8, arg_count: u8, ret_count: u8) {
        ...
        let call = HilExpr::Call {
            fun: Box::new(HilExpr::Local(func)),
            args,
        };
    ...
    fn lift_namecall(&mut self, dest: u8, object: u8, method: u32) {
        ...
        let method_call = HilExpr::MethodCall {
            object: Box::new(HilExpr::Local(object)),
            method,
            args,
        };
        ...
        let (values, has_variadic_tail) = match decoded_count(count) {
            ...
            Count::Variadic | Count::All => (
                self.take_variadic_from(base)
                    .unwrap_or_else(|| vec![HilExpr::Local(base)]),
                true,
            ),
        };
    ...
    fn emit_call_result(&mut self, dest: u8, ret_count: u8, expr: HilExpr) {
        match decoded_count(ret_count) {
            ...
            Count::Number(1) => self.assign(HilExpr::Local(dest), expr),
    ...
    /// later than `first`. The result is a fixed-prefix of `HilExpr::Local`
